...
if size(G.coords,2) == 2
    ...
    ind= find(isnan(signal) == 0);
    
    scatter3(G.coords(ind,1),G.coords(ind,2),G.coords(ind,3),...
                    param.vertex_size,signal(ind),'.');
    ind1=find(isnan(signal) == 1);
    plot3(G.coords(ind1,1),G.coords(ind1,2),G.coords(ind1,3),...
                    param.vertex_size/3,signal(ind1),'k.');
